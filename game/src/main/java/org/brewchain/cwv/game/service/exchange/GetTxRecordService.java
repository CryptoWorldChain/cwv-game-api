...
@Slf4j
@Data
public class GetTxRecordService extends SessionModules<ReqGetTxRecord> {
	...
	public String[] getCmds() {
		return new String[] { PExchangeCommand.GTR.name() };
	}
	...
	@Override
	public void onPBPacket(final FramePacket pack, final ReqGetTxRecord pb, final CompleteHandler handler) {
		
		pack.getExtHead().buildFor(pack.getHttpServerletResponse());
		ResGetTxRecord.Builder ret = ResGetTxRecord.newBuilder();
		try{
			walletHelper.getTxRecord(pack, pb, ret);
		}catch(Exception e){
