...
@Slf4j
@Data
public class CreateTxService extends SessionModules<ReqCreateTx> {
	...
	public String[] getCmds() {
		return new String[] { PExchangeCommand.CTS.name() };
	}
	...
	public void onPBPacket(final FramePacket pack, final ReqCreateTx pb, final CompleteHandler handler) {
		...
		ResCreateTx.Builder ret = ResCreateTx.newBuilder();
		try{
			walletHelper.createTx(pack, pb, ret);
		}catch(Exception e){
