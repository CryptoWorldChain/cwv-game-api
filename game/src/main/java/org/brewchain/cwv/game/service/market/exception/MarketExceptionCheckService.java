...
@Slf4j
@Data
public class MarketExceptionCheckService extends SessionModules<PSMarketExceptionCheck> {
...
//	
	...
	public String[] getCmds() {
		return new String[] { PMarketECommand.MEC.name() };
	}
	...
	@Override
	public void onPBPacket(final FramePacket pack, final PSMarketExceptionCheck pb, final CompleteHandler handler) {
		
		pack.getExtHead().buildFor(pack.getHttpServerletResponse());
		PRetCommon.Builder ret = PRetCommon.newBuilder();
		RetCodeMsg.Builder builder = RetCodeMsg.newBuilder();
		try{
			marketHelper.marketExceptionCheck(pb, ret, builder);
